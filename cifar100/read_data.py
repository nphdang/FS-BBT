...
# read data from file
def from_file(dataset="cifar100", type="original", plot=False):
    print("dataset: {}, type: {}".format(dataset, type))
    (X_train_org, y_train_org), (X_test_org, y_test_org) = cifar100.load_data()
    ...
    # get image size
    img_rows, img_cols, img_chns = X_train_org.shape[1:]
    # get no of features
    n_feature = img_rows * img_cols * img_chns
    # get image shape
    img_shape = (img_rows, img_cols, img_chns)
    print("original data")
    ...
    if type == "original":
        ...
        y_train = y_train_org
    if type == "subset":
        # keep balanced data but sample a subset (50 data points) of each label
        n_label_idx = 50
...
# X_train, y_train, X_test, y_test, n_train, n_test, n_feature, n_class, img_shape = from_file("cifar100", "subset", plot=True)
