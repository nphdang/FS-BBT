...
parser.add_argument('--cvaemethod', default="input_cond", type=str, nargs='?', help='method when training cvae')
parser.add_argument('--cvaelatent', default="2", type=str, nargs='?', help='latent when training cvae')
parser.add_argument('--cvaebatchsize', default="64", type=str, nargs='?', help='batch size when training cvae')
parser.add_argument('--cvaeepochs', default="600", type=str, nargs='?', help='epochs when training cvae')
parser.add_argument('--balance', default="0.5", type=str, nargs='?', help='trade-off factor between teacher predictions and true labels')
parser.add_argument('--batchsize', default="512", type=str, nargs='?', help='batch size to train student')
parser.add_argument('--epochs', default="50", type=str, nargs='?', help='epochs to train student')
...
print("gpu_id: {}, "
      ...
      "cvae_data_type: {}, cvae_method: {}, cvae_latent: {}, cvae_batch_size: {}, cvae_epochs: {}, "      
      ...
      format(args.gpu,
             ...
             args.cvaedatatype, args.cvaemethod, args.cvaelatent, args.cvaebatchsize, args.cvaeepochs,
...
student_model_type = 'resnet{}_light_kd'.format(depth)
...
if number_generate > 0:
    ...
    decoder = load_model("./{}/cvae_{}_decoder_{}_{}_method_{}_la{}_bs{}_ep{}_teacher_{}.h5".
                         format(load_reconstructed_distilled_data, teacher_model_type, dataset,
                                cvae_data_type, cvae_method, cvae_latent, cvae_batch_size, cvae_epochs,
                                teacher_data_type))
...
student = resnet_model.resnet_v1(input_shape=img_shape, depth=depth, num_classes=n_class)
if loss_type == "ce":
    student.compile(loss='categorical_crossentropy', optimizer=optimizers.Adam(lr=lr_schedule(0)), metrics=['accuracy'])
if loss_type == "kd":
    student.compile(loss=kd_loss, optimizer=optimizers.Adam(lr=lr_schedule(0)), metrics=['accuracy'])
...
if data_augmentation == "none":
    ...
    if loss_type == "kd":
        hist = student.fit(X_ce, y_ce_y_kl, batch_size=batch_size, epochs=epochs,
                           validation_data=(X_test, y_test_y_teacher), shuffle=True, callbacks=callbacks)
if data_augmentation == "standard":
    ...
    if loss_type == "kd":
        hist = student.fit_generator(datagen.flow(X_ce, y_ce_y_kl, shuffle=True, batch_size=batch_size),
                                     epochs=epochs, verbose=1, validation_data=(X_test, y_test_y_teacher), workers=4, callbacks=callbacks)
...
if data_augmentation == "mixup":
    ...
    if loss_type == "kd":
        ...
        hist = student.fit_generator(generator=mxgen, steps_per_epoch=X_ce.shape[0] // batch_size,
                                     epochs=epochs, verbose=1, validation_data=(X_test, y_test_y_teacher), callbacks=callbacks)
...
with open(save_path, 'w') as f:
  ...
  f.write("cvae_data_type: {}, cvae_method: {}, cvae_latent: {}, cvae_batch_size: {}, cvae_epochs: {}\n".
          format(cvae_data_type, cvae_method, cvae_latent, cvae_batch_size, cvae_epochs))
